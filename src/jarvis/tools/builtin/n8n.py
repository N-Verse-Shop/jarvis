...
class N8NTool(Tool):
    ...
    def description(self) -> str:
        # R35-S3 P2-12: description used to start with "Используй когда
        # пользователь ГОВОРИТ:" — keyword-matched poorly when the same
        # user TYPED the same phrase into the dashboard textbox. Phrasing
        # is now neutral ("говорит или пишет") + expanded synonyms cover
        # everyday RU ("что у меня настроено", "что работает") and the
        # frequent EN-mix ("workflow", "automation").
        return (
            "Управляй автоматизациями (workflow / automation) на n8n: список "
            "текущих, просмотр истории запусков, ручной запуск webhook, "
            "пауза/возобновление, создание новой автоматизации из шаблона, "
            "удаление (с подтверждением). "
            "Используй когда пользователь говорит ИЛИ пишет: "
            "'какие автоматизации', 'какие у меня workflows', "
            "'что у меня настроено', 'что работает', "
            "'запусти автоматизацию ...', 'настрой ...', 'останови автоматизацию ...', "
            "'покажи логи автоматизации ...', 'удали автоматизацию ...', "
            "'покажи мои воркфлоу', 'процессы на n8n', "
            "'show me automations', 'list workflows'. "
